...
# 0.0 disables voids and restores original solid-model behaviour.
VOID_RATE = 0.0
...
def _pairs_scale():
    # 8 = full Moore neighbourhood. Keeps porous models comparable.
    if _pair_count[0] <= 0:
        return 0.0
            ...
            for y in range (CAunitsY):
                for x in range (CAunitsX):
                    ...
                    if x != 0 and y != 0 and x != CAunitsX-1 and y != CAunitsY - 1:
                        ...
                        centroidS = _safe_centroid(boxS)
                        
                        index = (CAunitsX*CAunitsY)*z + (y-1) * CAunitsX + x + 2
                        boxSE = self.guid[index-1]
                        centroidSE = _safe_centroid(boxSE)
                        
                        index = (CAunitsX*CAunitsY)*z + y * CAunitsX + x + 2
                        boxE = self.guid[index-1]
                        centroidE = _safe_centroid(boxE)
                        
                        index = (CAunitsX*CAunitsY)*z + (y+1) * CAunitsX + x + 2
                        boxNE = self.guid[index-1]
                        centroidNE = _safe_centroid(boxNE)
                        
                        index = (CAunitsX*CAunitsY)*z + (y+1) * CAunitsX + x + 1
                        boxN = self.guid[index-1]
                        centroidN = _safe_centroid(boxN)
                        
                        index = (CAunitsX*CAunitsY)*z + (y+1) * CAunitsX + x
                        boxNW = self.guid[index-1]
                        centroidNW = _safe_centroid(boxNW)
                        
                        index = (CAunitsX*CAunitsY)*z + y * CAunitsX + x
                        boxW = self.guid[index-1]
                        centroidW = _safe_centroid(boxW)
                        
                        index = (CAunitsX*CAunitsY)*z + (y-1) * CAunitsX + x
                        boxSW = self.guid[index-1]
                        centroidSW = _safe_centroid(boxSW)
                        
                        # works out the sum of the distances between the neighbouring units
                        dist1 = _pair_dist(centroidThis, centroidS)
                        dist2 = _pair_dist(centroidThis, centroidSE)
                        dist3 = _pair_dist(centroidThis, centroidE)
                        dist4 = _pair_dist(centroidThis, centroidNE)
                        dist5 = _pair_dist(centroidThis, centroidN)
                        dist6 = _pair_dist(centroidThis, centroidNW)
                        dist7 = _pair_dist(centroidThis, centroidW)
                        dist8 = _pair_dist(centroidThis, centroidSW)
                        ...
                        THISxS = rs.BooleanIntersection(boxThis, boxS, False) if (boxThis is not None and boxS is not None) else None
                        if (THISxS != [] and THISxS is not None):
                            XXareasFactor += rs.SurfaceArea(THISxS)[0]
                            objects.append(THISxS)
                            #XXfactor += 100
                            
                        THISxSE = rs.BooleanIntersection(boxThis, boxSE, False) if (boxThis is not None and boxSE is not None) else None
                        if (THISxSE != [] and THISxSE is not None):
                            XXareasFactor += rs.SurfaceArea(THISxSE)[0]
                            objects.append(THISxSE)
                            #XXfactor += 100
                        
                        THISxE = rs.BooleanIntersection(boxThis, boxE, False) if (boxThis is not None and boxE is not None) else None
                        if (THISxE != [] and THISxE is not None):
                            XXareasFactor += rs.SurfaceArea(THISxE)[0]
                            objects.append(THISxE)
                            #XXfactor += 100
                            
                        THISxNE = rs.BooleanIntersection(boxThis, boxNE, False) if (boxThis is not None and boxNE is not None) else None
                        if (THISxNE != [] and THISxNE is not None):
                            XXareasFactor += rs.SurfaceArea(THISxNE)[0]
                            objects.append(THISxNE)
                            #XXfactor += 100
                            
                        THISxN = rs.BooleanIntersection(boxThis, boxN, False) if (boxThis is not None and boxN is not None) else None
                        if (THISxN != [] and THISxN is not None):
                            XXareasFactor += rs.SurfaceArea(THISxN)[0]
                            objects.append(THISxN)
                            #XXfactor += 100
                            
                        THISxNW = rs.BooleanIntersection(boxThis, boxNW, False) if (boxThis is not None and boxNW is not None) else None
                        if (THISxNW != [] and THISxNW is not None):
                            XXareasFactor += rs.SurfaceArea(THISxNW)[0]
                            objects.append(THISxNW)
                            #XXfactor += 100
                            
                        THISxW = rs.BooleanIntersection(boxThis, boxW, False) if (boxThis is not None and boxW is not None) else None
                        if (THISxW != [] and THISxW is not None):
                            XXareasFactor += rs.SurfaceArea(THISxW)[0]
                            objects.append(THISxW)
                            #XXfactor += 100
                            
                        THISxSW = rs.BooleanIntersection(boxThis, boxSW, False) if (boxThis is not None and boxSW is not None) else None
